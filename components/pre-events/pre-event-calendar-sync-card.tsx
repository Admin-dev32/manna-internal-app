...
import { initialPreEventCalendarSyncFormState } from '@/services/pre-events/calendar-form-state';
...
export function PreEventCalendarSyncCard({
  sync,
  requirements,
  action,
}: {
  ...
  requirements: string[];
  action: (state: PreEventCalendarSyncFormState, formData: FormData) => Promise<PreEventCalendarSyncFormState>;
}) {
  const [state, formAction] = useActionState(action, initialPreEventCalendarSyncFormState);
  ...
  const statusLabel =
    sync?.sync_status === 'reconciled'
      ...
      : sync?.sync_status === 'stale'
        ? 'Obsoleto (ownership transferido a Evento)'
        : isSynced
          ? 'Sincronizado'
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Google Calendar en reserva
        </CardTitle>
        <CardDescription>Sincronización manual para no competir ciegamente con el sistema maestro de webhooks.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="rounded-2xl border border-border bg-muted/20 p-4 text-sm">
          ...
          <p className="mt-1">External event id: {sync?.external_event_id ?? 'Sin registro'}</p>
          <p className="mt-1">External event url: {sync?.external_event_url ?? 'Sin registro'}</p>
